...
class FileMetadataMapper(AbstractDomainEntityMapper[FileMetadataDto, FileOrm]):
    def to_entity(self, domain_obj: FileMetadataDto) -> FileOrm:
        try:
            payload = {
                "id": domain_obj.id,
    ...
    def to_domain(self, entity_obj: FileOrm) -> FileMetadataDto:
        try:
            payload = {
                "id": entity_obj.id,
